...
## Everything comes from one photograph

All the imagery is derived from a single supplier render of the blue room
(`scripts/source/room-blue.png`), so every layer keeps that render's
lighting and perspective and the stack looks like one photograph:

- **Rooms** — the render with the door hardware, shower head and valve
  inpainted away: a clean, open shower with its pan. Green and grey are the
  same image with the wall paint hue-shifted.
- **Door and fixtures** — the real rail, rollers, handle, head and valve cut
  out of the render as transparent layers, then recoloured for matte black
  and brushed nickel. Chrome is the untouched original.
- **Walls** — each texture is multiplied onto the render's own wall
  luminance, so shading and reflections survive under sandstone or marble.
- **Tub, seat, shelves, caddy** — white acrylic forms built from the
  render's shower-pan pixels, so they match the pan's tone.
- **Grab bar** — the render's real chrome towel bar, keyed off the wall and
  moved into the shower.

The render shows one design — **shower, smooth white walls, sliding glass
door, chrome** — and stacking those layers reproduces it, so that is
`DEFAULT_CONFIGURATION` and the builder opens with it pre-selected (the
in-home tool opens the same way). Because every step starts with a value,
progress counts steps the user has **reviewed** (opened).

A different room render goes in `scripts/source/room-blue.png`; the
hardware positions at the top of the generator script then need
re-measuring.
...
Every image is generated by `scripts/generate-builder-placeholders.py`
from the source render (see above). The synthesised pieces — window,
accent band, tub spout, shelves — are the least photographic. Replace any
file by dropping a real image at the **same path**; no code changes needed.
...
- The window, accent band and tub spout are synthesised rather than photographed; real product layers on the same canvas replace them.
